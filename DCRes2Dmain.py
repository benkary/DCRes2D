...
AVsigx = 1./(np.dot(AVx.todense(),1./Sigma)); AVsigx = np.ravel(AVsigx, order='F')
AVsigy = 1./(np.dot(AVy.todense(),1./Sigma)); AVsigy = np.ravel(AVsigy, order='F')
...
Jac = DIV * ssp.diags(GRAD * np.ravel(V, order='F'), 0) * diagAVsig**2 * 
